...
import {
  ...
  IdolGroup,
  DateFormats,
  // A global like environment.ProcessorCount
  threadCount,
...
const Nogizaka46_Members = {
  '３期生': [
    '伊藤理々杏', '岩本蓮加', '梅澤美波', '大園桃子', '久保史緒里', '阪口珠美', '佐藤楓', '中村麗乃',
    '向井葉月', '山下美月', '吉田綾乃クリスティー', '与田祐希'
  ],
  '４期生': [
    '遠藤さくら', '賀喜遥香', '掛橋沙耶香', '金川紗耶', '北川悠理', '柴田柚菜', '清宮レイ', '田村真佑',
    '筒井あやめ', '早川聖来', '矢久保美緒'
  ],
  '新4期生': [
  ...
  '5期生': [
    '五百城茉央', '池田瑛紗', '一ノ瀬美空', '井上和', '岡本姫奈', '小川彩', '奥田いろは', '川﨑桜',
    '菅原咲月', '冨里奈央', '中西アルノ'
  ]
};
...
export async function Nogizaka46_Crawler() {
  ...
  const existingMembers = getMembers(Nogizaka46_BlogStatus_FilePath);
  ...
  const threadNumber = threadCount;
  for (let threadId = 0; threadId < threadNumber; threadId++) {
  ...
  const sortedEntries = Object.entries(Nogizaka46_Blogs).sort(
    (a, b) => a[1].DateTime - b[1].DateTime
  );
...
export function getGroupedMembers() {
  ...
  const groups = Object.keys(groupedByName).map(groupName => {
    ...
    if (Nogizaka46_Members[groupName]) {
